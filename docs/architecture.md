...
- `SegmentedControl`: tab-style segmented picker; used in Practice (hint mode) and Prep (answer approaches). Renders 2, 3, or 4 columns. The `layoutId="hintLevel"` spring animation is shared across instances — use distinct `layoutId` props if both screens are ever mounted simultaneously.
...
- `POST /api/speak/prepare`: prepares direct or bridge-based Speak session, approaches, and speaking plan.
...
- App language owns UI/help/explanation data; Practice language owns practice prompts, speaking plans, sample answers, subtitles/audio, and imitation answer versions.
